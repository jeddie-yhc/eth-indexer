...
func init() {
	...
	if _, err := os.Stat(cfgFile); os.IsNotExist(err) {
		...
		ServerCmd.Flags().StringVar(&dbName, flags.DbName, "eth-db", "The database name")
		...
		ServerCmd.Flags().StringVar(&profilAddr, flags.PprofAddress, "0.0.0.0", "pprof HTTP server listening interface")
	} else {
